...
export const InteractiveHoverButton = React.forwardRef(({ children, className, ...props }, ref) => {
  return (
    (<button
      ref={ref}
      className={cn(
        "group relative bg-primary cursor-pointer overflow-hidden py-6 px-6 xl:py-6 xl:px-10 text-center font-semibold",
        className
      )}
      {...props}>
      <div className="flex items-center gap-2">
        <div
          className="h-2 w-2 rounded-full bg-black transition-all duration-300 group-hover:scale-[100.8]"></div>
        <span
